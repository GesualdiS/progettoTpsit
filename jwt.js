...
function getTokens(id_user){
    ...
    db.query(`INSERT INTO User_tokens(id_user, refresh_token) VALUES (?, ?);`, [id_user, refreshToken], (err, results, fields) => {
        if (err) {
...
function refreshAccessToken(refreshToken){
    const id_user = null;
    db.query(`SELECT id_user FROM User_token WHERE refresh_token = ?;`, [email, refreshToken], (err, results, fields) => {
        if (err) {
            console.error('Error during the query:', err);
